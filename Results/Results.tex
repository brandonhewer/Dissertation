...
\par
Appropriately the identification of dinatural transformations, that occur in practice, begins with one of the most ubiquitous higher-order functions; filter. Observe the type of the \lstinline{filter} function
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}
filter :: (a -> Bool) -> [a] -> [a].
...
\end{center}
\par
As demonstrated, \lstinline{filter} is indeed a dinatural transformation. Continuing the examination of common higher-order functions, it is appropriate to consider the \lstinline{foldr} function. Observe the type of the \lstinline{foldr} function
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}
foldr :: (a -> b -> b) -> b -> [a] -> b.
...
\end{tabular}
\end{center}
Another higher-order parametrically polymorphic type, which operates on lists, is the \lstinline{span} function. Observe the type of the \lstinline{span} function
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}
span :: (a -> Bool) -> [a] -> ([a], [a])
...
\end{tabular}
\end{center}
The \lstinline{groupBy} is a higher-order function which groups elements of a list, by means of a relation. Observe the type of the \lstinline{groupBy} function
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}
...
\end{center}
\noindent
The \lstinline{insertBy} function, in the Haskell programming language, has a total of five occurrences of a type variable. Observe the type of the \lstinline{insertBy} function
\begin{center}
\begin{tabular}{c}
\begin{lstlisting}
...
\end{tabular}
\end{center}
This section will be concluded by exploring a few exotic types. The provided examples appear as the types of mutual recursion schemes. In particular, dicatamorphisms and prepromorphisms. Observe the type of the \lstinline{dicata} function
\begin{center}
\begin{tabular}{c}
...
Given an object $X$ of a category $\mathbb{C}$, currying is known to facilitate an adjunction between the functors $- \times X : \mathbb{C} \rightarrow \mathbb{C}$ and $X \rightarrow - : \mathbb{C} \rightarrow \mathbb{C}$. In this section, the impact of currying, on Petri nets generated within the application, will be explored.
\par
...
\end{tabular}
\end{center}
The \lstinline{curry} and \lstinline{uncurry} functions, appear to `bend' the graph, by pulling a node up or down and changing its variance. The idea of extranaturality resulting from `bending' graphs of naturality conditions was first discovered by \citeasnoun{GeneralTransform}.
