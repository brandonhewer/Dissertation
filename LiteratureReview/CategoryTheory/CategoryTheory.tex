...
\end{flalign}
An example of a functor, given in the Haskell programming language, is the list functor, shown in \lstref{Haskell-List}.
\begin{lstlisting}[
...
eval f x = f x
\end{lstlisting}
\lstref{Haskell-Eval} depicts an implementation of the evaluation map, which is extranatural in the type variable \lstinline{a}. By observation, the extranaturality condition can be verified; given any functions \lstinline{f : a' -> b, g : a -> a'}, the equality \lstinline{ (eval f) . g = eval (f . g)} holds.
\par
